  ...
  Widget build(BuildContext context) {
    final tabs = <String, Widget>{
      'Zones': const MasterCrudScreen(
        ...
        endpoint: '/api/zones',
        fields: [
          FieldSpec('zoneCode', 'Zone Code',
              hint: 'Example: Z1', required: false, maxLength: 20),
          FieldSpec('zoneName', 'Zone Name',
              hint: 'Example: North Zone',
              maxLength: 100,
              hindiKey: 'zoneNameHi'),
        ...
        columns: [
          ColumnSpec('id', 'Zone ID'),
          ColumnSpec('zoneCode', 'Code'),
          ColumnSpec('zoneName', 'Zone Name')
        ],
      ),
      'Villages': const MasterCrudScreen(
        ...
        fields: [
          FieldSpec('zoneId', 'Zone',
              type: FieldType.dropdown,
              optionsEndpoint: '/api/zones',
              optionLabelKey: 'zoneName'),
          FieldSpec('villageName', 'Village Name',
              hint: 'Example: Rampur',
              maxLength: 100,
              hindiKey: 'villageNameHi'),
          FieldSpec('pradhanName', 'Pradhan Name',
              hint: 'Example: Suresh Singh', required: false, maxLength: 100),
          FieldSpec('mobile', 'Mobile',
              hint: 'Example: 9876543210',
              type: FieldType.mobile,
              required: false),
          FieldSpec('email', 'Email',
              hint: 'Example: pradhan@gmail.com',
              type: FieldType.email,
              required: false),
        ],
        columns: [
          ColumnSpec('id', 'Village ID'),
          ColumnSpec('villageName', 'Village'),
          ColumnSpec('zoneName', 'Zone'),
          ColumnSpec('pradhanName', 'Pradhan'),
      ...
      'Banks': const MasterCrudScreen(
        ...
        endpoint: '/api/banks',
        fields: [
          FieldSpec('bankName', 'Bank Name',
              hint: 'Example: State Bank of India', maxLength: 100),
          FieldSpec('branchName', 'Branch Name',
              hint: 'Example: Rampur Branch', maxLength: 100),
          FieldSpec('ifsc', 'IFSC',
              hint: 'Example: SBIN0001234', maxLength: 11),
          FieldSpec('address', 'Address',
              hint: 'Branch address', required: false, maxLength: 250),
          FieldSpec('managerName', 'Manager Name',
              required: false, maxLength: 100),
          FieldSpec('managerMobile', 'Manager Mobile',
              hint: 'Example: 9876543210',
              type: FieldType.mobile,
              required: false),
          FieldSpec('managerEmail', 'Manager Email',
              hint: 'Example: manager@sbi.co.in',
              type: FieldType.email,
              required: false),
        ],
        columns: [
          ColumnSpec('id', 'ID'),
          ColumnSpec('bankName', 'Bank'),
          ColumnSpec('branchName', 'Branch'),
          ColumnSpec('ifsc', 'IFSC'),
          ColumnSpec('managerName', 'Manager'),
        ],
      ),
      'Vehicle Types': const MasterCrudScreen(
        ...
        fields: [
          FieldSpec('vehicleTypeName', 'Vehicle Type Name',
              hint: 'Example: Truck',
              maxLength: 50,
              hindiKey: 'vehicleTypeNameHi')
        ],
        columns: [
          ColumnSpec('id', 'ID'),
          ColumnSpec('vehicleTypeName', 'Vehicle Type')
        ],
      ),
      'Variety Types': const MasterCrudScreen(
        title: 'Variety Type',
        module: 'VarietyType',
        endpoint: '/api/variety-types',
        fields: [
          FieldSpec('varietyTypeName', 'Variety Type Name',
              hint: 'Example: Early', maxLength: 50)
        ],
        columns: [
          ColumnSpec('id', 'ID'),
          ColumnSpec('varietyTypeName', 'Variety Type')
        ],
      ),
      'Varieties': const MasterCrudScreen(
        title: 'Variety',
        module: 'Variety',
        endpoint: '/api/varieties',
        fields: [
          FieldSpec('varietyTypeId', 'Variety Type',
              type: FieldType.dropdown,
              optionsEndpoint: '/api/variety-types',
              optionLabelKey: 'varietyTypeName'),
          FieldSpec('varietyName', 'Variety Name',
              hint: 'Example: CO 18', maxLength: 50),
        ],
        columns: [
          ColumnSpec('id', 'ID'),
          ColumnSpec('varietyTypeName', 'Variety Type'),
          ColumnSpec('varietyName', 'Variety')
        ],
      ),
      ...
      'Parties': const MasterCrudScreen(
        ...
        fields: [
          FieldSpec('partyName', 'Party Name',
              hint: 'Example: Gupta Traders',
              maxLength: 100,
              hindiKey: 'partyNameHi'),
          FieldSpec('mobile', 'Mobile',
              hint: 'Example: 9876543210', type: FieldType.mobile),
          FieldSpec('email', 'Email',
              hint: 'Example: party@gmail.com',
              type: FieldType.email,
              required: false),
          FieldSpec('address', 'Address', required: false, maxLength: 250),
          FieldSpec('gst', 'GST Number',
              hint: 'Optional GSTIN', required: false, maxLength: 15),
        ],
        columns: [
          ColumnSpec('id', 'ID'),
          ColumnSpec('partyName', 'Party'),
          ColumnSpec('mobile', 'Mobile'),
          ColumnSpec('gst', 'GST'),
        ],
      ),
      'Seasons': const MasterCrudScreen(
        ...
        fields: [
          FieldSpec('seasonName', 'Season Name',
              hint: 'Example: 2026-27', maxLength: 20),
          FieldSpec('startDate', 'Start Date (yyyy-MM-dd)',
              hint: 'Example: 2026-10-01'),
          FieldSpec('isActive', 'Active Season',
              type: FieldType.toggle, required: false),
        ],
        columns: [
          ColumnSpec('id', 'ID'),
          ColumnSpec('seasonName', 'Season'),
      ...
      'Payment Modes': const MasterCrudScreen(
        ...
        fields: [
          FieldSpec('modeCode', 'Mode Code',
              hint: 'Example: CASH', maxLength: 20),
          FieldSpec('modeName', 'Mode Name',
              hint: 'Example: Cash', maxLength: 50),
        ],
        columns: [
          ColumnSpec('id', 'ID'),
          ColumnSpec('modeCode', 'Code'),
          ColumnSpec('modeName', 'Name')
        ],
      ),
      'Loan Types': const LoanTypesScreen(),
      'Expense List': const MasterCrudScreen(
        title: 'Expense List',
        module: 'ExpenseType',
        endpoint: '/api/expense-types',
        fields: [
          FieldSpec('expenseName', 'Expense Name',
              hint: 'Example: Petrol, Diesel, Labor',
              maxLength: 100,
              hindiKey: 'expenseNameHi'),
          FieldSpec('description', 'Description',
              hint: 'Optional description', required: false, maxLength: 250),
        ],
        columns: [
          ColumnSpec('id', 'ID'),
          ColumnSpec('expenseName', 'Expense Name'),
...
class _RateScreenState extends State<RateScreen> {
  ...
  Future<void> _load() async {
    setState(() => _loading = true);
    try {
      final r1 = await ApiClient.instance.dio
          .get('/api/rates', queryParameters: {'includeInactive': true});
  ...
  Future<void> _newRate({Map<String, dynamic>? revisionOf}) async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setD) => AlertDialog(
          title: Text(
              revisionOf == null ? 'New Rate Period' : 'Revise Rate Period'),
          content: SizedBox(
            width: 400,
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              DropdownButtonFormField<int>(
                decoration: const InputDecoration(
                    labelText: 'Variety Type', hintText: 'Select Variety Type'),
                items: [
