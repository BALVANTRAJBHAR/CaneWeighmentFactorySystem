...
class _DeviceConfigScreenState extends State<DeviceConfigScreen> {
    ...
    try {
      ...
      setState(() {
        if (d.statusCode == 200) {
          ...
          _loadError = 'Could not load devices: ${ApiClient.errorMessage(d)}';
        }
      ...
      if (mounted) {
        setState(() => _loadError = ApiClient.exceptionMessage(
            error, 'Could not reach the server to load weighing devices.'));
      }
    }
  }

  Future<void> _createDefaultDevice() async {
    final res = await ApiClient.instance.dio.post('/api/devices', data: {
      ...
      'activeStringProfileId': _testProfileId,
    });
    if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    // A Developer is always permitted to manage the digitizer.  Some older
    // server databases issued a token whose permissions list was incomplete
    // even though its role was Developer, which made the actions menu vanish.
    // Backend endpoints still enforce Device.Configure for every mutation.
    final canConfigure =
        auth.can('Device.Configure') || auth.hasRole('Developer');
      ...
      Card(
        child: Padding(
          ...
          child:
            ...
            if (_devices.isEmpty && canConfigure) ...[
              const SizedBox(height: 8),
              FilledButton.icon(
                  onPressed: _createDefaultDevice,
                  icon: const Icon(Icons.add),
                  label: const Text('Create Default Weighing Device')),
            ],
