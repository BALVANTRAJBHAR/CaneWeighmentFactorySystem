...
public class RtspCaptureProvider : ICameraCaptureProvider, ICameraContinuousStreamProvider
{
    ...
    /// <summary>
    /// Resolves ffmpeg consistently for an interactive developer machine, an
    /// IIS deployment, or a service account. The deployed API first uses its
    /// bundled tools\ffmpeg\ffmpeg.exe; PATH is only the final fallback.
    /// </summary>
    private static string ResolveFfmpegPath()
    {
        var configured = Environment.GetEnvironmentVariable("CANE_FFMPEG_PATH");
        if (!string.IsNullOrWhiteSpace(configured) && File.Exists(configured)) return configured;

        var bundled = Path.Combine(AppContext.BaseDirectory, "tools", "ffmpeg", "ffmpeg.exe");
        if (File.Exists(bundled)) return bundled;

        var besideApi = Path.Combine(AppContext.BaseDirectory, "ffmpeg.exe");
        if (File.Exists(besideApi)) return besideApi;
