        ...
        var document = Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().Column(col =>
                {
                    ...
                    if (doc.IsDuplicate)
                        col.Item().PaddingTop(6).Border(1.5f).BorderColor(Colors.Red.Medium)
                            .Background(Colors.Red.Lighten4).PaddingVertical(4).AlignCenter()
                            .Text("DUPLICATE / REPRINT • प्रतिलिपि")
                ...
                page.Content().PaddingTop(10).Column(content =>
                {
                    ...
                    foreach (var detailTable in doc.Tables.Where(t => t.Columns.Count > 0 && t.Rows.Count > 0))
                    {
                        content.Item().PaddingTop(12).Text(hindi ? detailTable.TitleHindi : detailTable.TitleEnglish)
                            .FontSize(10).Bold().FontColor(Colors.Blue.Darken2);
                        content.Item().PaddingTop(4).Table(table =>
                        {
                            table.ColumnsDefinition(columns =>
                            {
                                foreach (var _ in detailTable.Columns) columns.RelativeColumn();
                            });
                            table.Header(header =>
                            {
                                foreach (var column in detailTable.Columns)
                                    header.Cell().Background(Colors.Blue.Darken2).Padding(3)
                                        .Text(hindi ? column.LabelHindi : column.LabelEnglish).FontSize(8).Bold().FontColor(Colors.White);
                            });
                            foreach (var detailRow in detailTable.Rows)
                            {
                                for (var columnIndex = 0; columnIndex < detailTable.Columns.Count; columnIndex++)
                                {
                                    var value = columnIndex < detailRow.Count ? detailRow[columnIndex] : "-";
                ...
                page.Footer().PaddingTop(8).Column(col =>
                {
                    ...
                    col.Item().AlignCenter().PaddingTop(2).Text(t =>
                    {
                        t.Span(doc.Language == "hi" ? "कंप्यूटर जनरेटेड पर्ची" : "This is a computer-generated slip.")
                            .FontSize(7).FontColor(Colors.Grey.Medium);
                        t.Span("  •  Warrior Softech").Bold().FontSize(7).FontColor(Colors.Grey.Medium);
                    });
                });
            });
        });
