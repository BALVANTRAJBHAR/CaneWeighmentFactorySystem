...
public class ConfigController : ControllerBase
{
        ...
        CancellationToken ct)
    {
        ...
        {
            // Do not let a missing external decoder become an unhandled API
            // exception. Before the multipart response starts we can return a
            // useful client error; after it starts the stream simply closes.
            HttpContext.RequestServices.GetRequiredService<ILogger<ConfigController>>()
                .LogError(ex, "Live camera {CameraNumber} could not start ffmpeg. Configure CANE_FFMPEG_PATH or deploy the bundled decoder.", camera.CameraNumber);
            if (!Response.HasStarted)
            {
                Response.StatusCode = StatusCodes.Status503ServiceUnavailable;
                await Response.WriteAsJsonAsync(new { message = "Camera decoder (ffmpeg.exe) is missing on this API server." }, ct);
            }
        }
    }
