...
class _AppShellState extends State<AppShell> {
  ...
  static final List<_NavItem> _allItems = [
    ...
    _NavItem('Audit Log', Icons.history_outlined, 'Audit.View', () => const AuditScreen()),
    _NavItem('User Guide', Icons.menu_book_outlined, 'UserGuide.View', () => const UserGuideScreen()),
  ];
  ...
  Future<void> _loadHeader() async {
    try {
      ...
      if (res.statusCode == 200 && mounted) setState(() => _header = Map<String, dynamic>.from(res.data));
    } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (items.isEmpty) {
      return Scaffold(
          body: Center(
              child: Text('No modules available for your account. Contact your administrator.',
    ...
    return Scaffold(
      appBar: AppBar(
        titleSpacing: 12,
        title: Row(children: [
          Icon(Icons.factory_outlined, color: Theme.of(context).colorScheme.primary),
          ...
          Flexible(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('Season: ${_header?['season'] ?? '-'}', style: const TextStyle(fontSize: 11)),
            ]),
          ),
        ]),
        actions: [
          if (wide)
            Padding(
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.end, children: [
                Text(DateFormat('dd MMM yyyy').format(_now), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)),
                Text(DateFormat('HH:mm:ss').format(_now), style: const TextStyle(fontSize: 12)),
              ]),
            ),
          Padding(
          ...
          IconButton(
            ...
            onPressed: () => theme.setMode(theme.mode == ThemeMode.dark ? ThemeMode.light : ThemeMode.dark),
          ),
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (_) => const [
              ...
              PopupMenuItem(value: 'logoutAll', child: Text('Logout All Sessions')),
            ],
          ),
          const SizedBox(width: 8),
        ],
        ...
        NavigationRail(
          ...
          destinations: [
            for (final i in items)
              NavigationRailDestination(icon: Icon(i.icon), label: Text(i.label, textAlign: TextAlign.center)),
          ],
        ),
