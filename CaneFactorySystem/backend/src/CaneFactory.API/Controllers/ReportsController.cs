...
public class ReportsController : ControllerBase
{
        ...
        [FromQuery] string? status, [FromQuery] string format = "json", [FromQuery] int page = 1, [FromQuery] int pageSize = 100)
    {
        ...
        if (fromDate.HasValue) q = q.Where(x => x.TareDateTime >= fromDate || (x.GrossDateTime != null && x.GrossDateTime >= fromDate));
        if (toDate.HasValue) q = q.Where(x => x.TareDateTime <= toDate || (x.GrossDateTime != null && x.GrossDateTime <= toDate));
        ...
        var tableRows = rows.Select(x => new List<string> { x.salePurchaseId.ToString(), x.item, x.party, x.vehicleType, x.VehicleNumber, x.driver,
            ...
            x.TareDateTime.ToString("dd-MM-yyyy HH:mm"), x.GrossDateTime?.ToString("dd-MM-yyyy HH:mm") ?? "-", x.status }).ToList();
